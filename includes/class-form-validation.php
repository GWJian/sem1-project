        ...
        foreach( $rules as $key => $condition )
        {
            switch( $condition )
            {
                ...
                case 'password_check';
                    ...
                    // step 2: make sure length is at least 4 characters
                    else if(strlen( $data[$key] ) < 4){
                ...
                case 'update_product_form';
                    ...
                    break;
                case 'cart_form';
                    if(!CSRF::verifyToken( $data[$key],'cart_form' )){
                        $error .= 'Invalid CSRF Token<br/> ';
                    }
                    break;
                case 'delete_cart_form';
                    if(!CSRF::verifyToken( $data[$key],'delete_cart_form' )){
                        $error .= 'Invalid CSRF Token<br/> ';
                    }
                    break;
            }
